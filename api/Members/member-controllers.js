...
//Controller Logic (member-controllers.js)
...
exports.getMembers = async (req, resp) => {
    try {
        ...
        const { count, rows } = await memberModel.findAndCountAll({
            ...
            include: [{ model: NomineeModel, as: 'nominees', attributes: ['name', 'relationship', 'name2', 'relationship2'] }],
            limit,
            offset
        });
        ...
        console.log(error);
        resp.status(500).json({ message: 'Error retrieving members' });
    }
}

exports.updateMember = async (req, resp) => {
    try {
        await REST_API._update(req, resp, memberModel);
    } catch (error) {
        resp.status(500).json({ error: 'Failed to update member' });
    }
}

exports.deleteMember = async (req, resp) => {
    try {
        await REST_API._delete(req, resp, memberModel);
    } catch (error) {
        resp.status(500).json({ error: 'Failed to delete member' });
    }
}
...
exports.getMemberById = async (req, resp) => {
    try {
        ...
        await REST_API._getDataListById(req, resp, memberModel, key, value);
    } catch (error) {
        resp.status(500).json({ error: 'Error retrieving member details' });
    }
}

exports.membergetsByPagination = async (req, resp) => {
    try {
        await REST_API.Pagination(req, resp, memberModel);
    } catch (error) {
        resp.status(500).json({ error: 'Error retrieving paginated members' });
    }
}
...
exports.forgetPassword = async (req, resp) => {
    try {
        ...
        await Mail.sendEmail(MemberEmail[0].email, 'For Forget Password', `YOUR FORGET PASSWORD OTP IS ${otp}`);
        await REST_API._forget(req, resp, ForgetModel, otp);
    } catch (error) {
        resp.status(500).json({ error: 'Failed to handle password reset' });
    }
}
...
exports.varify = async (req, resp) => {
    try {
        ...
        if (varify) {
            await ForgetModel.destroy({ where: { member_id: req.params.id } });
            await REST_API._updatePassword(req, resp, memberModel);
        } else {
            resp.status(500).json({ message: 'OTP not found' });
        }
    } catch (error) {
...
exports.otp = async (req, resp) => {
    try {
        ...
        if (emailRegex.test(req.body.via)) {
            await Mail.sendEmail(req.body.via, 'For OTP Verify', `Your OTP is ${otp}`);
            await REST_API.saveOTP(req, resp, OtpModel, otp);
        } else {
            resp.status(400).json({ message: 'Invalid email format' });
        }
    } catch (error) {
        resp.status(500).json({ error: 'Failed to generate OTP' });
    }
}

exports.verifyOTP = async (req, resp) => {
    try {
        await REST_API.OTPVERIFYMEMBER(req, resp, OtpModel, req.body.otp);
    } catch (error) {
        resp.status(500).json({ error: 'Failed to verify OTP' });
    }
}
...
const test = async (req, resp, id) => {
    ...
    const uploadedFile2 = req.files?.file2;
    const profile = req.files?.profile;

    if (!uploadedFile && !uploadedFile2 && !profile) {
